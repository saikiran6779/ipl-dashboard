import { Card, TeamChip, EmptyState, Spinner } from '../components/UI'
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, letterSpacing: 2, color: '#f97316' }}>All Matches</h2>
        <span style={{ fontSize: 12, color: '#8b949e' }}>{matches.length} match{matches.length !== 1 ? 'es' : ''} recorded</span>
      </div>
      ...
      {!matches.length ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {matches.map((m, i) => (
            <div key={m.id} className="fade-up" style={{ animationDelay: `${i * 0.04}s`, background: '#161b22', border: '1px solid #21262d', borderRadius: 14, padding: 16 }}>
              {/* Meta row */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => onEdit(m)} style={{ padding: '4px 10px', background: '#21262d', border: '1px solid #30363d', borderRadius: 6, color: '#8b949e', cursor: 'pointer', fontSize: 12 }}>✏️ Edit</button>
                  <button onClick={() => onDelete(m.id)} style={{ padding: '4px 10px', background: '#21262d', border: '1px solid #30363d', borderRadius: 6, color: '#ef4444', cursor: 'pointer', fontSize: 12 }}>🗑️</button>
                </div>
              </div>
